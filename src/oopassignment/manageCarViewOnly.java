...
public class manageCarViewOnly extends javax.swing.JFrame {
    ...
    public manageCarViewOnly(Customer LoggedInCustomer) {
    ...
    private void returnPageActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_returnPageActionPerformed
        ...
        Main.manageCarViewOnly.setVisible(false);
    ...
    private void bookCarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bookCarActionPerformed
        ...
        if (count == 0) {
            JOptionPane.showMessageDialog(this, "Please select a car to book.", "No Selection", JOptionPane.WARNING_MESSAGE);
        } else if (count > 1) {
            JOptionPane.showMessageDialog(this, "You can only book ONE car at a time.", "Multiple Selections", JOptionPane.ERROR_MESSAGE);
        } else {
            String carID = (String) selectedRow[1]; 
            String customerID = LoggedInCustomer.getCustomerID();
            boolean hasBooked = false;
            boolean carFound = false;
   
            for (booking b : DataIO.bookings) {
                if (b.getCustomerID().equals(LoggedInCustomer.getCustomerID()) &&
                        b.getCarID().equals(carID) && 
            ...
            if (hasBooked){
                JOptionPane.showMessageDialog(this,
                            "You have already requested to book this car.\nPlease wait for a salesman to respond.",
            ...
            for (car c : DataIO.cars){
                if(c.getCarID().equals(carID) && c.getCarStatus().equals("Available")){
                    Main.requestBooking = new requestBooking(this, true, LoggedInCustomer, c);
                    Main.requestBooking.setVisible(true);
                    carFound = true;
                    break;
                }
            }
            if (!carFound){
                JOptionPane.showMessageDialog(this,
                        "Car not found or car is not available for booking.",
                        "Unavailable Car",
                        JOptionPane.WARNING_MESSAGE);
                    return;
            }
