...
public class viewPaymentCustomer extends javax.swing.JFrame {
    ArrayList<Object[]> originalPaymentData;
    oopassignment.PaymentCustomerTableModel paymentModel;
    private Customer LoggedInCustomer;
    
    public viewPaymentCustomer(Customer LoggedInCustomer) {
        this.LoggedInCustomer = LoggedInCustomer;
        paymentModel = new oopassignment.PaymentCustomerTableModel(DataIO.payments, LoggedInCustomer);
        originalPaymentData = paymentModel.getBookingData();
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        paymentTable.setFont(new java.awt.Font("Arial", 0, 18));
        paymentTable.setModel(paymentModel);
        jScrollPane1.setViewportView(paymentTable);
        TableStyler.stylePaymentCustomerTable(paymentTable, jScrollPane1);
    ...
    private void returnPageActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_returnPageActionPerformed
        ...
        Main.viewPaymentCustomer.setVisible(false);
    }//GEN-LAST:event_returnPageActionPerformed

    private void searchBarKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_searchBarKeyReleased
        String keyword = searchBar.getText().toLowerCase();
        ArrayList<payment> searchedPayments = new ArrayList<>();

        for (Object[] row : originalPaymentData) {
            if (rowContains(row, 0, 11, keyword)) {
                String paymentID = (String) row[0];
                for (payment p : DataIO.payments) {
                    if (p.getPaymentID().equals(paymentID)) {
                        searchedPayments.add(p);
                        break;
                    }
                }
            }
    ...
    public void refreshPage(ArrayList<payment> payments){
        paymentModel= new PaymentCustomerTableModel(payments, LoggedInCustomer);
        paymentTable.setModel(paymentModel);
        TableStyler.stylePaymentCustomerTable(paymentTable, jScrollPane1);
    }
